...
```{r, echo=FALSE, results='asis'}
print(dfSummary(data, plain.ascii = FALSE, graph.magnif = 0.75, headings=FALSE), method="render")
...
Por su parte, para medir el origen cultural de los estudiantes (considerando etnia y nacionalidad), generamos tres variables. Primeramente, la etnia y nacionalidad por separado de modo que podamos cuantificar la cantidad de estudiantes indigenas y migrantes que hay en la matricula secundaria a nivel nacional. Sólo 4.5% de la matricula es de una nacionalidad diferente a la chilena, mientras un 16.6% tiene origen índigena. Cabe relevar que para la nacionalidad y etnia (por lo tanto, también para variable cultura), los casos pérdidos se codificaron con la categoría "2" que la clasificamos como "missing". La razón de ello estriba en la importancia de no perder más observaciones adicionales a aquellas fruto de la limpieza de datos. 
...
##probamos interaccion bajo nse sd ingresos matematica

results_7 = lmer(puntaje_mate ~ 1 + sd_ingresos + bajo_nse + sd_ingresos*bajo_nse + log(ingresos) + sexo + educacion + nacionalidad + etnia + m_educacion + sd_educacion + m_ingresos + nacionalidad_j + etnia_j + sexo_j + (1|rbd), data = data_agg)

screenreg(results_7, naive=TRUE)

plot_model(results_7, type="int")
...
htmlreg(list(results_1, results_2, results_3),
    ...
    custom.coef.names = c("Intercepto", "Ingresos" ,"Mujer", "Educación", "Migrante", "Indígena", "Heterogeneo socialmente", "Homogeneidad social media", "Homogeneidad social baja", "Heterogeneidad educativa", "Heterogeneidad cultural", "Homogeneidad cultural alta", "Homogeneidad étnica baja", "Heterogeneo étnicamente", "Baja homogeneidad género", "Alta homogeneidad género" ),
    custom.note = "%stars. Errores estándar en paréntesis",
    caption="Comparación modelos",
    caption.above=TRUE,
    digits=2,
    doctype = FALSE,
    override.coef= list(c(198.35,4.05,-5.45,1.97,3.04,0.09), c(289.45,45.04,13.64,28.8,-22.44,-2.46,-12.56,7.18,-0.29,-8.43,0.79), c(227.18,3.42,-5.63,1.86,2.98,0.34,38.4,11.85,25.45,-19.05,-3.87,-15.13,5.65,-1.16,-9.93,3.09)),
    override.pvalues= list(c(0,0,0,0,0,0.9), c(0,0,0,0,0,0.9,0,0.009,0.9,0,0.9), c(0,0,0,0,0,0.9,0,0,0,0,0.009,0,0.009,0.15,0,0.7)))

##aun falta arreglar errores estandar de los coeficientes que no coinciden con el screen reg
```


En primer lugar, antes de comenzar a describir los párametros de los modelos estimados, es importante mencionar la cantidad de varianza de los puntajes de cada una de las pruebas que hay entre las escuelas chilenas (técnicamente denominada correlación intraclase que se obtiene a partir del modelo nulo). Siguiendo los resultados, tenemos que un 47% de la varianza del puntaje SIMCE Matemática está entre las escuelas. Para el caso de lenguaje e historia la varianza entre escuelas es más baja, del orden de 29% y 32% respectivamente. 

Si estimamos el modelo de regresión sólo considerando las variables individuales los datos nos muestran que todas las variables que teoricamente formulamos como potencialmente explicativas del rendimiento academico en matemática lo son empiricamente. En ese sentido, todas las variables incluidas son estadisticamente significativas con un nivel de confianza equivalente a un 99%. Las variables que muestran una relación más fuerte con el puntaje SIMCE de Matematica 

desaparece efecto heterogeneidad cuando ingreso media socioeconomica, para educacion cuando meto promedio pierde potencia la heterogeneidad educativa. ahora bien si pongo solo sd ingreso y sd educacion tienen tres estrellitas. 
